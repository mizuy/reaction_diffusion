#!/usr/bin/env python3
"""Interactive simulator for saturation / environment-C pit models (Step 1c, Step 3).

Uses evaluate_pit_steps.calc_step with reaction saturation, cubic damping,
optional uniform-core or A-depletion terms, and static or dynamic environment C.

Example:
    uv run python src/exp3.py
    uv run python src/exp3.py --preset step1c_c_uniform_core
    uv run python src/exp3.py --preset step3_pit_vi_dynamic_environment
"""
...
LIVE_METRICS: tuple[tuple[str, str, int], ...] = (
    ...
    ("vi_irregularity_score", "vi_irreg", 2),
...
SNAPSHOT_DIR = Path("artifacts/exp3_snapshots")
DEFAULT_PRESET = "step1d_pit_iiil_spatial_sat"

EXP3_PRESETS: tuple[str, ...] = (
    "step1b_pit_iv_saturated_cubic",
    "step1c_a_strong_cubic",
    "step1c_b_a_depletion",
    "step1c_c_uniform_core",
    "step1d_pit_iiil_spatial_sat",
    "step1d_b_pit_iiil_spatial_refractory",
    "step3_pit_vi_dynamic_environment",
)

PRESET_MENU_LABELS: dict[str, str] = {
    "step1b_pit_iv_saturated_cubic": "step1b sat+cubic",
    "step1c_a_strong_cubic": "step1c_a strong μ",
    "step1c_b_a_depletion": "step1c_b A depl",
    "step1c_c_uniform_core": "step1c_c core",
    "step1d_pit_iiil_spatial_sat": "step1d spatial q",
    "step1d_b_pit_iiil_spatial_refractory": "step1d q+R",
    "step3_pit_vi_dynamic_environment": "step3 dyn C",
}

_CORE = ("feed", "k", "d_a", "d_b")
_SAT = ("reaction_saturation", "cubic_damping")
_SPATIAL_SAT = (
    "reaction_saturation",
    "reaction_saturation_tip",
    "cubic_damping",
    "uniform_core_decay",
)
_REFRACTORY = ("refractory_rate", "refractory_decay", "refractory_strength")
_1C_B = ("a_depletion",)
_1C_C = ("uniform_core_decay",)
_ENV = ("static_env_scale", "feed_env_sensitivity", "k_env_sensitivity")
_ENV_DYN = _ENV + ("env_rate", "env_diffusion", "env_source", "env_decay")

PRESET_PARAM_ATTRS: dict[str, tuple[str, ...]] = {
    "step1b_pit_iv_saturated_cubic": _CORE + _SAT,
    "step1c_a_strong_cubic": _CORE + _SAT,
    "step1c_b_a_depletion": _CORE + _SAT + _1C_B,
    "step1c_c_uniform_core": _CORE + _SAT + _1C_C,
    "step1d_pit_iiil_spatial_sat": _CORE + _SPATIAL_SAT,
    "step1d_b_pit_iiil_spatial_refractory": _CORE + _SPATIAL_SAT + _REFRACTORY,
    "step3_pit_vi_dynamic_environment": _CORE + _SAT + _ENV_DYN,
}
...
PARAM_SPECS: list[tuple[str, str, float, float, float, int]] = [
    ...
    ("reaction_saturation", "q core", 0.0, 3.0, 1.5, 2),
    ("reaction_saturation_tip", "q tip", 0.0, 2.0, 0.5, 2),
    ("cubic_damping", "B³ μ", 0.0, 0.05, 0.018, 4),
    ("a_depletion", "A depl η", 0.0, 0.5, 0.0, 3),
    ("uniform_core_decay", "core δ", 0.0, 0.3, 0.10, 3),
    ("refractory_rate", "R rate", 0.0, 0.15, 0.0, 3),
    ("refractory_decay", "R decay", 0.0, 0.2, 0.0, 3),
    ("refractory_strength", "R kill", 0.0, 0.4, 0.0, 3),
    ("static_env_scale", "C scale", 0.0, 2.0, 0.0, 2),
    ("feed_env_sensitivity", "C→feed", 0.0, 0.01, 0.0, 4),
    ("k_env_sensitivity", "C→k", -0.01, 0.01, 0.0, 4),
    ("env_rate", "C ε", 0.0, 0.1, 0.0, 3),
    ("env_diffusion", "C D", 0.0, 1.0, 0.0, 2),
    ("env_source", "C α", 0.0, 2.0, 0.0, 2),
    ("env_decay", "C β", 0.0, 0.5, 0.0, 3),
]
...
def param_attrs_for_case(case_name: str, base_config: ModelConfig) -> tuple[str, ...]:
    ...
    attrs: list[str] = list(_CORE + _SAT)
    if base_config.reaction_saturation_tip > 0:
        attrs.extend(_SPATIAL_SAT)
    elif base_config.uniform_core_decay > 0:
        attrs.extend(_CORE + _SAT + _1C_C)
    if base_config.a_depletion > 0:
        attrs.extend(_1C_B)
    if base_config.refractory_strength > 0 or base_config.refractory_rate > 0:
        attrs.extend(_REFRACTORY)
    if base_config.static_env_scale > 0 or base_config.dynamic_env:
        attrs.extend(_ENV_DYN if base_config.dynamic_env else _ENV)
    return tuple(dict.fromkeys(attrs))


def exp3_suite() -> dict[str, ModelConfig]:
    all_steps = {cfg.name: cfg for cfg in build_suite("steps")}
    return {name: all_steps[name] for name in EXP3_PRESETS if name in all_steps}


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Saturation / environment-C pit simulator (exp3)."
    )
    parser.add_argument(
        "--preset",
        choices=EXP3_PRESETS,
...
class EnvironmentSimulator:
    def __init__(
        ...
        *,
        ...
        self.view_c = Viewport(2 * VIEW_W, 0, SCREEN_W, VIEW_H, self.w, self.h)
        self.screen_size = (SCREEN_W, SCREEN_H)

        pygame.init()
        self.screen = pygame.display.set_mode(self.screen_size)
        pygame.display.set_caption("Pit pattern — exp3 sat/env (3×2)")
        self.font_small = pygame.font.Font(None, 22)
        self.clock = pygame.time.Clock()
        self.ui = pygame_gui.UIManager(self.screen_size)

        self.suite = exp3_suite()
    ...
    def _sync_param_visibility(self) -> None:
        ...
        for attr in SLIDER_ATTRS:
            show = attr in visible
            (self.param_labels[attr].show if show else self.param_labels[attr].hide)()
            (self.param_sliders[attr].show if show else self.param_sliders[attr].hide)()
    ...
    def model_config(self) -> ModelConfig:
        data = {f.name: getattr(self.base_config, f.name) for f in fields(ModelConfig)}
        for attr in self.visible_param_attrs:
            data[attr] = self._slider(attr)
        data["name"] = self.active_case
        data["description"] = "exp3 live"
    ...
    def reset_fields(self) -> None:
        ...
        self.grid_c = build_static_environment(
            self.h, self.settings.seed, cfg.static_env_scale
        )
    ...
    def _uses_refractory_view(self, cfg: ModelConfig) -> bool:
        return cfg.refractory_strength > 0.0 or cfg.refractory_rate > 0.0

    def _aux_caption_lines(self, cfg: ModelConfig) -> list[str]:
        if self._uses_refractory_view(cfg):
            return [
                "refractory R (old B residence)",
                f"rate {cfg.refractory_rate:.3f}  decay {cfg.refractory_decay:.3f}  "
                f"kill {cfg.refractory_strength:.3f}",
                f"R std {float(self.grid_r.std()):.3f}",
            ]
        mode = "dynamic C" if cfg.dynamic_env else "static C"
        lines = [
            f"environment C ({mode})",
            f"C scale {cfg.static_env_scale:.2f}  →feed {cfg.feed_env_sensitivity:.4f}  "
            f"→k {cfg.k_env_sensitivity:.4f}",
        ]
        if cfg.dynamic_env:
            lines.append(
                f"ε {cfg.env_rate:.3f}  D {cfg.env_diffusion:.2f}  "
                f"α {cfg.env_source:.2f}  β {cfg.env_decay:.3f}"
            )
        lines.append(f"C std {float(self.grid_c.std()):.3f}")
        return lines
    ...
    def _render(self) -> None:
        ...
        self.view_ab.caption(
            ...
            (
                ...
                f"q_core {cfg.reaction_saturation:.2f}  "
                f"q_tip {cfg.reaction_saturation_tip:.2f}  "
                f"μ {cfg.cubic_damping:.4f}\n"
        ...
        self.view_skeleton.caption(
            ...
            "\n".join(
                [
                    "skeleton  white=skel  red=3-arm fork  cyan=end",
                    *self.metrics_lines(),
                ]
            ),
            self.font_small,
        ...
        aux = np.zeros((self.h, self.w, 3), dtype=np.uint8)
        if self._uses_refractory_view(cfg):
            aux[:, :, 0] = field_to_u8(self.grid_r)
        else:
            aux[:, :, 1] = field_to_u8(self.grid_c)
        self.view_c.blit_rgb(self.screen, aux)
        self.view_c.caption(
            self.screen,
            "\n".join(self._aux_caption_lines(cfg)),
            self.font_small,
        )
