#!/usr/bin/env python3
"""Interactive prototype: polarity field P + anisotropic B diffusion (Step 1e).

Example:
    uv run python src/exp4.py
    uv run python src/exp4.py --preset step1e_c_pit_iv_polarity_1d
"""
...
SNAPSHOT_DIR = Path("artifacts/exp4_snapshots")
DEFAULT_PRESET = "step1e_pit_iv_polarity_aniso"

EXP4_PRESETS: tuple[str, ...] = (
    "step0_pit_iv_baseline",
    "step1e_pit_iv_polarity_aniso",
    "step1e_b_pit_iv_polarity_1c",
    "step1e_c_pit_iv_polarity_1d",
)

PRESET_MENU_LABELS: dict[str, str] = {
    "step0_pit_iv_baseline": "step0 baseline",
    "step1e_pit_iv_polarity_aniso": "step1e P+aniso",
    "step1e_b_pit_iv_polarity_1c": "step1e +1c_c",
    "step1e_c_pit_iv_polarity_1d": "step1e +1d",
}
...
PRESET_PARAM_ATTRS: dict[str, tuple[str, ...]] = {
    ...
    "step1e_pit_iv_polarity_aniso": _CORE + _POLARITY,
    "step1e_b_pit_iv_polarity_1c": _CORE + _SAT + _1C_C + _POLARITY,
    "step1e_c_pit_iv_polarity_1d": _CORE + _SPATIAL_SAT + _POLARITY,
}
...
PARAM_SPECS: list[tuple[str, str, float, float, float, int]] = [
    ...
    ("d_b", "dB along P", 0.1, 2.0, 0.5, 2),
    ("polarity_diffusion", "P diff", 0.0, 0.5, 0.12, 3),
    ("polarity_align_rate", "P→∇B", 0.0, 0.3, 0.10, 3),
    ("polarity_decay", "P decay", 0.0, 0.2, 0.04, 3),
    ("d_b_across", "dB across", 0.0, 0.5, 0.22, 3),
    ("anisotropic_strength", "aniso mix", 0.0, 1.0, 0.45, 2),
    ("reaction_saturation", "q core", 0.0, 3.0, 1.6, 2),
...
def polarity_to_rgb(p_x: np.ndarray, p_y: np.ndarray) -> np.ndarray:
    """R/G = P components, B = |P| (growth direction field)."""
    mag = np.sqrt(p_x * p_x + p_y * p_y)
    scale = float(np.percentile(mag, 99.0)) or 1.0
    rgb = np.zeros((p_x.shape[0], p_x.shape[1], 3), dtype=np.uint8)
    rgb[:, :, 0] = field_to_u8(p_x, 95.0)
    rgb[:, :, 1] = field_to_u8(p_y, 95.0)
    rgb[:, :, 2] = np.clip(mag / scale * 255.0, 0, 255).astype(np.uint8)
    return rgb
...
def param_attrs_for_case(case_name: str, base_config: ModelConfig) -> tuple[str, ...]:
    ...
    attrs = list(_CORE)
    if uses_polarity(base_config):
        attrs.extend(_POLARITY)
    if base_config.reaction_saturation_tip > 0.0:
        attrs.extend(_SPATIAL_SAT)
    elif base_config.reaction_saturation > 0.0 or base_config.cubic_damping > 0.0:
        attrs.extend(_SAT)
        if base_config.uniform_core_decay > 0.0:
            attrs.append("uniform_core_decay")
    return tuple(dict.fromkeys(attrs))


def exp4_suite() -> dict[str, ModelConfig]:
    all_cases = {cfg.name: cfg for cfg in build_suite("steps")}
    return {name: all_cases[name] for name in EXP4_PRESETS}


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Polarity + anisotropic diffusion (Step 1e)")
    parser.add_argument("--preset", default=DEFAULT_PRESET, help="Model preset name")
    ...
    parser.add_argument("--metrics-frames", type=int, default=60)
    parser.add_argument(
        "--from-metrics",
        type=Path,
        default=None,
        help="Load case from artifacts/pit_step_eval/metrics.json",
    )
...
def resolve_startup(args: argparse.Namespace) -> tuple[ModelConfig, EvalRunSettings]:
    ...
    if args.from_metrics is not None:
        config, metrics_settings = load_eval_case(args.from_metrics, args.preset)
        return config, EvalRunSettings(
            size=args.size if args.size != DEFAULT_EVAL_RUN.size else metrics_settings.size,
...
class PolaritySimulator:
    def __init__(
        ...
        *,
        ...
        self.view_p = Viewport(2 * VIEW_W, 0, SCREEN_W, VIEW_H, self.w, self.h)
        self.screen_size = (SCREEN_W, SCREEN_H)

        pygame.init()
        self.screen = pygame.display.set_mode(self.screen_size)
        pygame.display.set_caption("Pit pattern — exp4 polarity + aniso (3×2)")
        self.font_small = pygame.font.Font(None, 22)
        self.clock = pygame.time.Clock()
        self.ui = pygame_gui.UIManager(self.screen_size)

        self.suite = exp4_suite()
    ...
    def _layout_param_sliders(self) -> None:
        ...
        if not attrs:
            return
        cols = min(SLIDER_COLS, len(attrs))
    ...
    def metrics_lines(self) -> list[str]:
        ...
        return ["  ".join(lines[0:4]), "  ".join(lines[4:])]
    ...
    def _polarity_caption_lines(self, cfg: ModelConfig) -> list[str]:
        mag = np.sqrt(self.grid_p_x ** 2 + self.grid_p_y ** 2)
        return [
            "polarity P (R=Px G=Py B=|P|)",
            f"P diff {cfg.polarity_diffusion:.3f}  align {cfg.polarity_align_rate:.3f}  "
            f"decay {cfg.polarity_decay:.3f}",
            f"dB along {cfg.d_b:.2f}  across {cfg.d_b_across:.3f}  |P| mean {float(mag.mean()):.3f}",
        ]
    ...
    def _render(self) -> None:
        ...
        self.view_ab.caption(
            ...
            (
                ...
                f"q {cfg.reaction_saturation:.2f}\n"
        ...
        if uses_polarity(cfg):
            p_rgb = polarity_to_rgb(self.grid_p_x, self.grid_p_y)
        else:
            p_rgb = np.zeros((self.h, self.w, 3), dtype=np.uint8)
        self.view_p.blit_rgb(self.screen, p_rgb)
        self.view_p.caption(
            self.screen,
            "\n".join(self._polarity_caption_lines(cfg)),
            self.font_small,
        )

        panel = self._panel_rect()
        hint = self.font_small.render(
            "P aligns to ∇B; B diffuses fast along P, weak across P",
            True,
            (120, 120, 120),
        )
...
def main(argv: list[str] | None = None) -> None:
    ...
    PolaritySimulator(config, settings, metrics_frame_interval=args.metrics_frames).run()
